...
$.fn.lightBox = function (options) {
	...
	if (util.is_options_valid(options)) {
		obj = new constructor(options,this);
        this.data('lt_Box', obj);
	}	
...
var proto_lightbox = (function () {
	...
	window.modExposed.stack = lt.stack;
	...
	/** option = {index:0,all:false,noAnim:false}*/
	$.fn.lightBox.close = function(option){
		var start = option.all?(lt.stack.length-1):(option.index||0);
		var end = option.all?-1:(start-1);
    ...
    var resizeLtBox = function () {
        ...
        var availH = innerH - parseInt(this.options.dimens.height == 'auto' ? this.options.ltBox.height() : this.options.dimens.height),
            tpPos = 0;
        var availW = innerW - parseInt(this.options.dimens.width == 'auto' ? this.options.ltBox.width() : this.options.dimens.width),
            lpPos = 0;
        if (availH > 0) {			
    ...
    var close = function (index,noAnim) {
		...
		if(!lt.stack.length)
			return;
		lt.stack.splice(index||0,1);
		
		/** Close callback Obj */	
		var callBackObj = {
			cb:function(){	
				util.switchClass.call(_this.options.ltBox,'lightbox_open','lightbox')
				if(lt.stack.length){				
					var top = lt.stack[0];	
