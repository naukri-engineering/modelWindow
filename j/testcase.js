...
test( "LightBox should be of HTML node type ", function() {  	
	var map = [{elem : $(window),value : "null",msg:'Windows type element cannot be lightbox'},{elem : $(document),value : "null",msg: 'Document type element cannot be lightbox'},{elem : $('#one'),value : "object",msg:'HTML element can be lightbox'}]		
	for(key in map){
		if(map.hasOwnProperty(key)){
			var test_obj = map[key];

			var options = {ltBox:test_obj['elem']};
			var obj = $.fn.lightBox(options);			
...
test("Check util functions",function(){
	...
	var flag = m.util.is_animtion_support();
...
test("Single time opening and closing lightBox",function(){
	var obj  = $(window).lightBox({ltBox:$('#one'),open:{event:'dummy'}})	
	obj.open();
...
test("Closing lightBox from code",function(){
	...
	$.fn.lightBox.close({index:1})
	ok(m.stack[0].options.ltBox.selector=='#two',"Closed lightBox at 1 Index")

	$.fn.lightBox.close({index:0})
	ok(m.stack.length==0,"Closed lightBox at 0 Index")

	obj.open();
	obj2.open();

	$.fn.lightBox.close({all:true})
